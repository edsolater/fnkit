...
- 优先写“人一眼能懂”的代码与命名，避免不必要的抽象词（如无上下文的 `resolvedTarget`、`effectResult`）
- 能用一个清晰概念表达的，不拆成多层间接包装
- 注释先解释“为什么”，再解释“怎么做”；避免重复代码表面含义
- 局部命名要贴近业务语义（如 `finalObject`、`proxyTarget`），减少术语噪音
- 评估实现方案时，若功能等价，默认选择更短、更直白、更稳定的一版
