 ...
 */
export function toIterable<T>(target: Iterator<T> | Iterable<T> | IterableIterator<T>): IteratorObject<T> {
...
export function toCollectionIterator<C extends Collection>(
  ...
  if (isArray(collection)) {
    return toIterable(collection.values()).map((v, idx) => [v, idx]) as any
  } else if (isSet(collection)) {
    return toIterable(collection.values()).map((v, idx) => [v, idx]) as any
  } else if (isMap(collection)) {
    return toIterable(collection.entries()).map(([k, v]) => [v, k]) as any
  } else if (isIterable(collection)) {
    return toIterable(collection as any).map((v, idx) => [v, idx]) as any
  } else if (isObject(collection)) {
    return toIterable(Object.entries(collection)).map(([k, v]) => [v, k]) as any
  } else {
