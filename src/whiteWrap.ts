 ...
 * wrapFn(() => 3) //=> () => 3
 */
export const wrapFn = <T extends MayFn<any>>(v: T): (() => DeMayFn<T>) =>
  (isFunction(v) ? v : () => v) as any
 ...
 * wrapArr([3]) //=> [3]
 */
export const wrapArr = <T extends MayArray<any>>(v: T): DeMayArray<T>[] =>
  (isArray(v) ? v : [v]) as any
...
export const wrapObj = <T extends MayObj<any>, P extends string>(
  v: T,
  keyName: P
): Record<MayObjKey<T> | P, DeMayObj<T>> =>
  (isObject(v) ? v : { [keyName ?? 'defaultKey']: v }) as any
